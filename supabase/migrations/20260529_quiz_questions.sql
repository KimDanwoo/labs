...
-- ===== yeko =====
('yeko-lamb', 'yeko', '예코이가 진심으로 좋아하는 고기는?', $j$["양고기","삼겹살","닭갈비","소고기"]$j$::jsonb, 0, '오히려 향 나는 양고기를 더 선호할 정도로 진심입니다 🐑'),
('yeko-redbean', 'yeko', '예코이가 좋아하는 단 음식은?', $j$["팥붕어빵·비비빅·양갱 (단팥)","초콜릿","캐러멜","단 거 안 좋아함"]$j$::jsonb, 0, '단팥 들어간 건 다 좋아할 정도로 단팥파입니다 🫘'),
('yeko-game', 'yeko', '예코이가 게임 중에 유일하게 진심인 게임은?', $j$["크레이지 아케이드","리그 오브 레전드","배틀그라운드","오버워치"]$j$::jsonb, 0, '게임에 관심이 적은 편이지만 크아만큼은 진심입니다 💣'),
('yeko-mbti', 'yeko', '예코이의 MBTI에서 J 비율은?', $j$["99% (극J)","60% 정도","50/50 경계","P 우세"]$j$::jsonb, 0, '재검사에서 J 99%가 나온 극J, 마감 한참 전부터 일정 세분화 📅'),
('yeko-color', 'yeko', '예코이의 시그니처 색깔은?', $j$["파란색 💙","보라색 💜","분홍 💗","빨강 ❤️"]$j$::jsonb, 0, '돌고래와 어울리는 파란색이 예코이의 시그니처 🐬'),
('yeko-animal', 'yeko', '예코이를 대표하는 동물은?', $j$["돌고래","범고래","곰","여우"]$j$::jsonb, 0, '대표 동물은 돌고래, 그래서 별명도 돌고래·범고래 🐬'),
('yeko-drink', 'yeko', '예코이의 주량 순위는?', $j$["PLCO 1등","중간 정도","못 마심","술 안 마심"]$j$::jsonb, 0, '"술은 짝으로 사야 한다"고 인정 받은 1등 🍺'),
('yeko-soju', 'yeko', '예코이가 회식 때 고집하는 술은?', $j$["소주","맥주","와인","하이볼"]$j$::jsonb, 0, '낮에는 맥주, 밤·회식에는 무조건 소주만 취급 🍶'),
('yeko-noodle', 'yeko', '예코이가 좋아하는 라면은?', $j$["진라면 매운맛·틈새라면","신라면","안성탕면","너구리"]$j$::jsonb, 0, '매운 음식 좋아하는 예코이답게 매콤한 라면 라인 🌶️'),
('yeko-hate', 'yeko', '예코이가 가장 싫어하는 것은?', $j$["벌레·더운 날씨","추위","시끄러운 곳","단 음식"]$j$::jsonb, 0, '여름과 벌레를 진심으로 싫어합니다 🪲☀️'),
('yeko-shower', 'yeko', '예코이가 샤워할 때 특이한 습관은?', $j$["순서를 정해놓고 무조건 똑같이","매일 다르게","랜덤하게","샤워 안 함"]$j$::jsonb, 0, '레버 각도까지 똑같이 맞춰야 하루 루틴이 안 망가져요 🚿'),
('yeko-hobby', 'yeko', '예코이의 취미가 아닌 것은?', $j$["축구","기타 연주","러닝","커피"]$j$::jsonb, 0, '기타·러닝·커피·술이 예코이의 공식 취미 🎸'),
('yeko-height', 'yeko', '예코이의 키는?', $j$["183cm","170cm","178cm","190cm"]$j$::jsonb, 0, '183cm, B형, 처녀자리 ✨'),
('yeko-perfume', 'yeko', '예코이가 좋아하는 향수 스타일은?', $j$["중성 향수 (BYREDO Blanche)","달콤한 머스크","시트러스","향수 안 씀"]$j$::jsonb, 0, '향수 매니아, 중성적인 Blanche가 시그니처 🌬️'),
('yeko-tv', 'yeko', '예코이가 즐겨 보는 TV 장르는?', $j$["연애 프로그램 (나솔·환승연애·하트시그널)","다큐멘터리","뉴스","스포츠 중계"]$j$::jsonb, 0, '도파민이 터지는 연애 프로그램을 가리지 않습니다 💘'),
('yeko-realname', 'yeko', '예코이의 본명은?', $j$["남예준","도은호","한노아","채봉구"]$j$::jsonb, 0, '남예준, 그래서 "남리더"라는 별명도 생겼어요 👑'),
('yeko-bloodtype', 'yeko', '예코이의 혈액형은?', $j$["B형","O형","A형","AB형"]$j$::jsonb, 0, '혈액형 B, 처녀자리, 가을 태생 ✨'),
('yeko-dessert', 'yeko', '예코이가 좋아하는 디저트가 아닌 것은?', $j$["생크림 케이크","스콘","마카롱","인절미"]$j$::jsonb, 0, '단팥·인절미·꿀떡·스콘·마카롱 같은 한식+서양 디저트를 좋아해요 🍰'),
('yeko-snack', 'yeko', '예코이가 가장 좋아하는 간식은?', $j$["뭉친 김 (조미김)","초콜릿","과자","아이스크림"]$j$::jsonb, 0, '간식이 김이라니, 예코이답죠 🌿'),
('yeko-pineapple', 'yeko', '예코이가 싫어하는 음식은?', $j$["파인애플 피자","닭발","회","청국장"]$j$::jsonb, 0, '의외로 파인애플 피자에 대해선 단호하게 NO 🍕🚫'),
('yeko-skate', 'yeko', '예코이가 좋아하는 활동 중 하나는?', $j$["스케이트","스키","암벽등반","서핑"]$j$::jsonb, 0, '스케이트 타는 걸 좋아해요 ⛸️'),
('yeko-greeting', 'yeko', '예코이의 인사말은?', $j$["예로롱","예이~","예준이입니다","뽀로롱"]$j$::jsonb, 0, '"예준 + 포롱"이 합쳐진 시그니처 인사 🐬'),
('yeko-cafe-signoff', 'yeko', '예코이가 공식 카페 글 끝에 항상 쓰는 말은?', $j$["남예준 올림","예준 드림","예준이가","안녕히 계세요"]$j$::jsonb, 0, '편지 형식으로 정중하게 "남예준 올림"으로 마무리 💌'),
('yeko-bubble', 'yeko', '예코이의 특수 능력은?', $j$["물·거품을 다룸","불을 다룸","빛을 다룸","바람을 다룸"]$j$::jsonb, 0, '물 줄기를 화면에 그리고 입에서 거품이 나오는 능력 💦'),
('yeko-position', 'yeko', 'PLCO에서 예코이의 포지션은?', $j$["리더 & 메인 보컬","메인 댄서","메인 래퍼","서브 보컬"]$j$::jsonb, 0, 'PLCO의 리더이자 메인 보컬, 작사·작곡까지 하는 만능 🎤'),
...
('yeko-recruit', 'yeko', '예코이가 PLCO에 함께 데려온 멤버는?', $j$["노아·은호","밤비·하민","하민·밤비","은호·하민"]$j$::jsonb, 0, '첫 트레이니였고, 노아와 은호를 그룹에 합류시켰어요 🤝'),
('yeko-pork', 'yeko', '예코이가 국밥에 선호하는 고기는?', $j$["돼지고기 + 여러 가지","소고기 한 가지","닭고기","국밥 안 먹음"]$j$::jsonb, 0, '돼지고기 베이스에 여러 고기를 섞어 먹는 걸 좋아해요 🍲'),
('yeko-bug', 'yeko', '예코이가 진심으로 무서워하는 것은?', $j$["벌레","귀신","높은 곳","바다"]$j$::jsonb, 0, '벌레라면 진심으로 도망갑니다 🪲🏃'),
...
('ako-bringing', 'ako', '아코를 PLCO에 데려온 멤버는?', $j$["예코이","밤코","은코","하코이"]$j$::jsonb, 0, '예코이가 첫 트레이니로서 아코·은코를 모았어요 🤝'),
...
('bamko-partner', 'bamko', '밤코와 함께 안무를 짜는 멤버는?', $j$["하코이","예코이","아코","은코"]$j$::jsonb, 0, '댄스 라인 콤비, PLCO 안무 거의 다 둘이 만들어요 💃'),
...
('bamko-rival', 'bamko', '밤코가 가장 자주 티격태격하는 멤버는?', $j$["은코","예코이","아코","하코이"]$j$::jsonb, 0, '은코가 밤코를 PLCO에 영입해서 더 친한 만큼 자주 다퉈요 ⚡'),
('bamko-crazyarcade', 'bamko', '크레이지 아케이드에서 밤코의 위치는?', $j$["예코이를 이긴 적 있는 고수","입문자","못 함","관심 없음"]$j$::jsonb, 0, '예코이가 크아 진심인데, 밤코가 한 판 이긴 적이 있어요 🏎️'),
...
('bamko-dance-position', 'bamko', 'PLCO에서 밤코의 메인 포지션은?', $j$["메인 댄서 & 안무가","메인 보컬","메인 래퍼","리드 보컬"]$j$::jsonb, 0, '하코이와 함께 PLCO 안무 거의 다 만들어요 💃'),
...
('eunko-recruit', 'eunko', '은코가 PLCO에 합류시킨 멤버는?', $j$["밤코","예코이","아코","하코이"]$j$::jsonb, 0, '밤코를 데려온 게 은코, 그래서 자주 티격태격해요 ⚡'),
...
('eunko-recruit-bamko', 'eunko', '은코가 PLCO에 영입한 멤버는?', $j$["밤코","아코","하코이","예코이"]$j$::jsonb, 0, '밤코를 영입한 사람이 은코, 그 후로 자주 티격태격해요 ⚡'),
('eunko-recruit-by', 'eunko', '은코를 PLCO에 합류시킨 사람은?', $j$["예코이","아코","밤코","하코이"]$j$::jsonb, 0, '첫 트레이니였던 예코이가 아코와 함께 영입했어요 🤝'),
...
-- ===== hako =====
('hako-meal', 'hako', '하코이가 먹은 음식의 80%를 차지하는 것은?', $j$["라면","빵","김밥","햄버거"]$j$::jsonb, 0, '뭘 먹었냐 물어보면 매번 라면이라고 답해요 🍜'),
('hako-ramen', 'hako', '하코이가 가장 좋아하는 라면은?', $j$["스낵면","신라면","짜파게티","진라면"]$j$::jsonb, 0, '라면 중에서도 스낵면을 최애로 꼽았어요 ✨'),
('hako-mbti', 'hako', '하코이의 MBTI는?', $j$["INFJ-T","ENFP-T","ISTP-A","ENTP-T"]$j$::jsonb, 0, '낯가리는 INFJ, 막내인데 어른스러운 이유 🌙'),
('hako-color', 'hako', '하코이의 시그니처 색은?', $j$["검정 🖤","하양 🤍","빨강 ❤️","보라 💜"]$j$::jsonb, 0, '검은 고양이 + 블랙 하트가 하코이의 시그니처 🐈‍⬛'),
('hako-animal', 'hako', '하코이의 대표 동물은?', $j$["검은 고양이","흰 고양이","햄스터","강아지"]$j$::jsonb, 0, '검은 고양이가 하코이의 상징 🐈‍⬛'),
('hako-nickname', 'hako', '하코이의 별명은?', $j$["햄냥이·비타민","핑쬬","늑댕이","돌고래"]$j$::jsonb, 0, '햄냥이(hamnyangi)와 비타민(vithamin)이 대표 별명 🍊'),
('hako-height', 'hako', '하코이의 키와 멤버 중 순위는?', $j$["185cm, 가장 큼","170cm, 가장 작음","178cm, 중간","180cm, 두번째"]$j$::jsonb, 0, '185cm로 PLCO 최장신, 게다가 막내 📏'),
('hako-blood', 'hako', '하코이의 혈액형은?', $j$["AB형","A형","B형","O형"]$j$::jsonb, 0, '독특한 AB형 막내, 전갈자리 ♏'),
('hako-taekwondo', 'hako', '하코이의 무술 실력은?', $j$["태권도 검은띠","유도 갈색띠","복싱 챔피언","운동 못함"]$j$::jsonb, 0, '태권도 검은띠 보유자, 댄스에도 큰 도움이 돼요 🥋'),
('hako-art', 'hako', '하코이의 의외의 예술 취미는?', $j$["그림 그리기 (아크릴화 카페 전시)","뜨개질","도자기","서예"]$j$::jsonb, 0, '아크릴화를 그려서 Asterum 카페에 전시까지 했어요 🎨'),
('hako-trot', 'hako', '하코이가 의외로 잘 부르는 장르는?', $j$["트로트","오페라","재즈","메탈"]$j$::jsonb, 0, '트로트를 진심으로 좋아하고 잘 불러요 🎤'),
('hako-impression', 'hako', '하코이의 숨겨진 특기는?', $j$["성대모사","마술","주짓수","복화술"]$j$::jsonb, 0, '성대모사가 정확해서 자주 멤버들 따라해요 😼'),
('hako-hate', 'hako', '하코이가 가장 싫어하는 것은?', $j$["거짓말·모기","시끄러운 곳","추위","단 음식"]$j$::jsonb, 0, '거짓말과 모기를 진심으로 싫어해요 🦟'),
('hako-partner', 'hako', '하코이와 함께 안무를 짜는 멤버는?', $j$["밤코","예코이","아코","은코"]$j$::jsonb, 0, '댄스 라인 콤비, 안무 창작은 거의 둘이서 🕺'),
('hako-language', 'hako', '하코이가 구사할 수 있는 외국어는?', $j$["일본어","중국어","스페인어","독일어"]$j$::jsonb, 0, '일본어가 가능해서 일본 활동 때 빛나요 🇯🇵'),
('hako-likes', 'hako', '하코이가 좋아한다고 자주 말하는 것은?', $j$["음식·운동·칭찬·게임","잠·휴식·산책","독서·영화·여행","공부·일"]$j$::jsonb, 0, '특히 칭찬을 좋아해서 자주 칭찬 요청을 해요 🥰'),
('hako-realname', 'hako', '하코이의 본명은?', $j$["유하민","도은호","한노아","채봉구"]$j$::jsonb, 0, '본명 유하민, 본인은 "성 빼고 하민이라고 불러줘" 부탁 🐈‍⬛'),
('hako-zodiac', 'hako', '하코이의 별자리는?', $j$["전갈자리","쌍둥이자리","처녀자리","게자리"]$j$::jsonb, 0, '11월 1일생 전갈자리, AB형 🦂'),
('hako-position', 'hako', 'PLCO에서 하코이의 메인 포지션은?', $j$["메인 댄서 & 리드 래퍼","메인 보컬","리더","서브 보컬"]$j$::jsonb, 0, '댄스 라인 + 리드 래퍼, 막내인데 키도 가장 커요 💃'),
('hako-debut-order', 'hako', '하코이는 PLCO의 몇 번째 멤버로 공개됐나?', $j$["다섯 번째 (막내, 마지막 공개)","첫 번째","세 번째","네 번째"]$j$::jsonb, 0, '2023년 1월 16일 마지막 멤버로 공개된 막내 🐈‍⬛'),
('hako-recruit-by', 'hako', '하코이를 PLCO에 데려온 멤버는?', $j$["아코","예코이","은코","밤코"]$j$::jsonb, 0, '노아(아코)가 마지막 다섯 번째 멤버로 하코이를 데려왔어요 🤝'),
('hako-line', 'hako', '하코이는 어느 라인 소속?', $j$["Ye-line (예준 라인)","No-line","Producer Line","Dance Line만"]$j$::jsonb, 0, '예준의 "Ye-line" 소속이자 댄스 라인 멤버 🌙'),
('hako-chicken', 'hako', '하코이가 라면 다음으로 좋아하는 음식은?', $j$["치킨","피자","햄버거","회"]$j$::jsonb, 0, '라면 80% + 치킨이 하코이의 음식 라인업 🍗'),
('hako-ramen2', 'hako', '하코이가 스낵면 외에 좋아하는 라면은?', $j$["육개장 사발면·짜파게티","신라면 블랙","진라면 매운맛","오뚜기 컵누들"]$j$::jsonb, 0, '육개장 사발면도 짜파게티도 좋아해요 🍜'),
('hako-catchphrase', 'hako', '하코이가 자주 하는 시그니처 말은?', $j$["논중화물","예로롱","바지락칼국수","핑쬬"]$j$::jsonb, 0, '논쟁의 중심·화제의 물음표 = 논중화물 🔥'),
('hako-belt', 'hako', '하코이의 태권도 단증은?', $j$["검은띠 (블랙벨트)","갈색띠","파란띠","아무 단증 없음"]$j$::jsonb, 0, '진짜 검은띠 보유자, 댄스의 폭발력 비결 🥋'),
('hako-bowling', 'hako', '하코이의 의외 취미가 아닌 것은?', $j$["스킨스쿠버","볼링","그림","축구"]$j$::jsonb, 0, '볼링·그리기·영상 시청·축구가 공식 취미 🎳'),
('hako-mosquito', 'hako', '하코이의 천적은?', $j$["모기","벌","나비","거미"]$j$::jsonb, 0, '거짓말과 모기를 진심으로 싫어해요 🦟🚫'),
('hako-jp', 'hako', '하코이가 일본 활동에서 빛나는 이유는?', $j$["일본어 가능","일본 만화 매니아","일식 전문가","일본인 친구 많음"]$j$::jsonb, 0, '일본어 구사 가능, 일본 무대에서 자주 활약 🇯🇵'),
('hako-blackbelt', 'hako', '하코이가 가장 자주 요청하는 것은?', $j$["칭찬","간식","휴식","용돈"]$j$::jsonb, 0, '"칭찬받아야 더 잘해요"라는 말을 자주 해요 🥰'),
('hako-acrylic', 'hako', '하코이의 그림이 전시된 곳은?', $j$["Asterum 433-10 카페","서울 미술관","학교 전시회","본인 SNS만"]$j$::jsonb, 0, '아크릴화 한 점이 PLCO 공식 카페에 전시까지 됐어요 🎨')
