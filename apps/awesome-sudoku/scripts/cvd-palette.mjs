...
const HUES = ['sky', 'blue', 'indigo', 'violet', 'fuchsia', 'pink', 'rose', 'orange', 'amber', 'yellow', 'lime', 'emerald', 'teal', 'cyan', 'slate', 'stone'];

// 후보: hue별 (라이트 100|200|300 @0.75, 다크 700|800 @0.45|0.60)의 모든 조합
const candidates = [];
for (const hue of HUES)
  for (const ls of [100, 200, 300, 400])
    for (const ds of [600, 700, 800, 900])
      for (const da of [0.45, 0.6, 0.75]) {
        const lightKey = `${hue}-${ls}`, darkKey = `${hue}-${ds}`;
        if (!colors[lightKey] || !colors[darkKey]) continue;
        const light = blend(oklchToLinearRgb(colors[lightKey]), LIGHT_BG, 0.75);
        ...
        candidates.push({ hue, cls: `bg-${hue}-${ls}/75 dark:bg-${hue}-${ds}/${da * 100}`, labs });
      }
...
function pairDist(a, b) {
  ...
  for (const mode of ['normal', 'protan', 'deutan', 'tritan']) {
    ...
    min = Math.min(min, dE(a.labs[`L_${mode}`], b.labs[`L_${mode}`]) * w, dE(a.labs[`D_${mode}`], b.labs[`D_${mode}`]) * w);
  }
  return min;
}

// 탐욕 max-min: hue당 1개, 9개 선택. 여러 시작점 시도 후 최고 채택
...
for (const c of best.set) console.log(`  '${c.cls}',`);
// 모드별 최악 쌍 리포트
for (const mode of ['normal', 'protan', 'deutan', 'tritan']) {
