...
export const LeaderboardTable = memo<LeaderboardTableProps>(({ records, isLoading, currentUserId }) => {
  if (isLoading) {
    return (
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full">
        <caption className="sr-only">리더보드 순위표</caption>
        <thead>
          <tr
            className={cn(
              'text-left text-xs',
        ...
        <tbody>
          {records.map((record, index) => {
            ...
            const diffLabel =
              GAME_LEVEL_LABELS[record.difficulty as keyof typeof GAME_LEVEL_LABELS] || record.difficulty;
            ...
            return (
              ...
              >
                ...
                <td className="py-3">
                  <span className={cn('font-bold font-tabular', 'text-[rgb(var(--color-text-primary))]')}>{point}</span>
                </td>
                <td className="py-3 hidden sm:table-cell">
                  <span className={cn('font-mono font-tabular', 'text-[rgb(var(--color-text-secondary))]')}>
                    {formatTime(record.completionTime)}
                  </span>
                </td>
                <td className="py-3 hidden md:table-cell">
                  <span className={cn('text-sm', 'text-[rgb(var(--color-text-secondary))]')}>{diffLabel}</span>
                </td>
                <td className="py-3 hidden md:table-cell">
                  <span
                    className={cn(
                      ...
                      record.gameMode === GAME_MODE.KILLER
                        ? cn('bg-[rgb(var(--color-error-bg))]', 'text-[rgb(var(--color-error-text))]')
                        : cn('bg-[rgb(var(--color-bg-tertiary))]', 'text-[rgb(var(--color-text-secondary))]'),
                    )}
                  >
