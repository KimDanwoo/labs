 ...
 */
export function calculatePoint(input: PointInput): PointResult {
  ...
  const killerDeduction = isKiller ? KILLER_MODE_DEDUCTION : 0;
  const totalPoint = Math.max(min, Math.round((basePoint - killerDeduction) * 10) / 10);
