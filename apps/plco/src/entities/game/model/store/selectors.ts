...
export const pendingPoopsAtom = selectAtom(gameAtom, (s) => s.pendingPoops ?? []);
...
export const eggReadyCharacterIdAtom = selectAtom(gameAtom, (s) => s.eggReadyCharacterId);
...
export const hasFoodAtom = atom((get) => Object.values(get(inventoryAtom)).some((count) => count > 0));
...
export const isAllUnlockedAtom = atom((get) => {
  ...
  const unlocked = new Set([...(get(unlockedCharactersAtom) ?? []), characterId]);
...
export const roomTypeAtom = atom<RoomType>((get) => {
  ...
  if (get(activeModalAtom) === MODAL_TYPE.MEETING || get(meetingPlayFriendAtom)) return ROOM_TYPE.OUTDOOR;
