...
import { DANGER_THRESHOLD, GAME_STATUS, MAX_CLEANLINESS, MAX_HEARTS, MAX_HUNGER } from '@shared/constants';
import type { CharacterInfo, GameState } from '@shared/types';
...
export default function FriendCard({ character, state, onClick }: FriendCardProps) {
  ...
  return (
    ...
    >
      <div className="shrink-0">
        <CharacterSprite characterId={character.id} size={56} direction="down" />
      </div>

      <div className="flex-1 text-left min-w-0">
        <div className="flex items-center justify-between gap-2">
          <span className="text-sm font-black truncate" style={{ color: character.color }}>
          ...
          {isStarted ? (
            ...
            <span className="text-[10px] font-bold text-gray-400 shrink-0">NEW</span>
          )}
        </div>
        ...
        {isStarted ? (
          <div className="flex items-center gap-2 mt-1.5">
            ...
            {state.isSick && <span className="text-[10px] font-bold text-red-500">🤒</span>}
            {state.isSleeping && <span className="text-[10px] font-bold text-blue-400">💤</span>}
            {state.status === GAME_STATUS.DEAD && (
              <span className="text-[10px] font-bold text-gray-400">별이 됐어요</span>
            )}
          </div>
        ) : (
          <div className="text-[11px] text-gray-400 mt-0.5">이름을 지어주고 만나보기</div>
        )}
      </div>
    </button>
  );
