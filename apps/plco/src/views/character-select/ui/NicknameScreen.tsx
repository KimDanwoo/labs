...
export default function NicknameScreen() {
  ...
  const handleBack = () => {
    setStep(visibleCharacters.length > 0 ? CHARACTER_SELECT_STEP.INTRO : CHARACTER_SELECT_STEP.CAROUSEL);
  };
      ...
      <div className="relative flex flex-col items-center gap-8 w-full max-w-xs">
        <div className="relative">
          <div
            ...
            style={{ backgroundColor: character.color }}
          />
          <CharacterSprite characterId={character.id} size={120} direction="down" />
        </div>

        <div className="text-center">
          <span className="text-2xl">{character.emoji}</span>
          <h2 className="text-xl font-black mt-1 transition-colors duration-300" style={{ color: character.color }}>
            {character.name}
          </h2>
        </div>
