...
export default function CharacterCarousel() {
  ...
  const handlePrev = () => {
    setIndex((prev) => (prev - 1 + ALL_CHARACTER_LIST.length) % ALL_CHARACTER_LIST.length);
  };
  const handleNext = () => {
        ...
        <div className="flex items-center justify-center w-full gap-2 sm:gap-4">
          ...
          >
            ...
            <div className="relative pointer-events-none">
              <CharacterSprite characterId={current.id} size={100} direction="down" />
            </div>

            <div className="text-center space-y-1 pointer-events-none">
              <div className="text-xl">{current.emoji}</div>
              <h2 className="text-lg font-black" style={{ color: current.color }}>
                {current.name}
              </h2>
            </div>
          </div>
