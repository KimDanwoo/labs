...
export default function QuizAdminPage() {
  ...
  return (
    <div className="space-y-3">
      ...
      {visible.map((r) => (
        <Link key={r.id} href={`${ADMIN_ROUTE.quiz}/${r.id}`} className="card p-3 block btn-press">
          <div className="flex items-center gap-2">
            {!r.is_active && <span className="text-[10px] text-red">비활성</span>}
            <span className="ml-auto text-muted text-xs">→</span>
          </div>
          <p className="text-xs text-foreground mt-1 line-clamp-2">{r.question}</p>
        </Link>
      ))}

      {!isLoading && visible.length === 0 && <p className="text-xs text-muted text-center py-6">아직 문제가 없어요</p>}
    </div>
  );
}
