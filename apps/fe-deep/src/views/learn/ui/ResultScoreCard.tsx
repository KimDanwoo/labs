...
export function ResultScoreCard({ resultCounts, totalCount }: ResultScoreCardProps) {
  const correctRate = totalCount > 0 ? Math.round(((resultCounts.good + resultCounts.easy) / totalCount) * 100) : 0;
    ...
    <Card className="p-6 mb-6 shadow-sm">
      ...
      {totalCount > 0 && (
        <div className="mt-5">
          ...
          <p className="text-sm text-muted-foreground mt-2 text-center tabular-nums">정답률: {correctRate}%</p>
        </div>
      )}
    </Card>
  );
