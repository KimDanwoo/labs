...
export function FlashcardPage() {
  ...
  const startStudy = async () => {
    ...
    if (studyMode === 'review') {
      ...
      questions = questions.filter((q) => !allProgress[q.id]).slice(0, questionCount);
    } else {
      const dueIds = getDueCardIds();
      const dueRaw = dueIds.length > 0 ? await getQuestionsByIds(dueIds.slice(0, Math.ceil(questionCount / 2))) : [];
      ...
      if (remaining > 0) {
        ...
        const allProgress = getLocalProgress();
        newQuestions = candidates.filter((q) => !allProgress[q.id]).slice(0, remaining);
      }
  ...
  const retryFailedCards = () => {
    const failedIds = new Set(results.filter((r) => r.rating === 'again').map((r) => r.questionId));
