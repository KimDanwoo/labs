...
import { isDailyDone, markDailyDone, useCardStudySession } from '../model';
...
export function DailyPage() {
  ...
  useEffect(() => {
    // localStorage 조회를 async 안으로 미룬다 — 이펙트 본문에서 동기로 setState하면 하이드레이션 렌더와 어긋난다.
    async function loadDailyQuestions(): Promise<Question[] | null> {
      ...
      if (selected.length < DAILY_COUNT) {
        ...
        const newCards = candidates.filter((q) => !existingIds.has(q.id) && !allProgress[q.id]).slice(0, remaining);
    ...
    return (
      <div className="container mx-auto max-w-[42rem] px-4 py-8 sm:py-12">
        <div className="text-center py-16 animate-fade-in-up">
          ...
          <p className="text-muted-foreground mb-6">오늘 학습을 이미 마쳤습니다. 내일 다시 도전하세요.</p>
  ...
  if (phase === 'ready') {
    return (
      <div className="container mx-auto max-w-[42rem] px-4 py-8 sm:py-12 animate-fade-in">
        ...
        <p className="text-muted-foreground mb-8">매일 {DAILY_COUNT}문제, 꾸준히 실력을 쌓으세요.</p>

        <Card className="p-8 text-center shadow-sm">
          <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-full bg-primary/10">
            <Calendar className="h-8 w-8 text-primary" />
          </div>
          <p className="text-lg font-medium mb-2">오늘의 {questions.length}문제가 준비되었습니다</p>
          <p className="text-sm text-muted-foreground mb-6">복습 카드와 새 카드가 혼합되어 있습니다.</p>
