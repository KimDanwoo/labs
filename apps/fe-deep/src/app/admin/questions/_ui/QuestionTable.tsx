...
import type { Category, Question } from '@entities/question/model';
...
export function QuestionTable({
  ...
  if (loading) {
    return <div className="p-8 text-center text-muted-foreground border rounded-lg">불러오는 중...</div>;
  }

  if (questions.length === 0) {
    return <div className="p-8 text-center text-muted-foreground border rounded-lg">결과가 없습니다.</div>;
  }
            ...
            {questions.map((q) => {
              ...
              return (
                <tr key={q.id} className="hover:bg-muted/30">
                  ...
                  <td className="p-3 whitespace-nowrap">{cat ? `${cat.icon} ${cat.title}` : q.category_id}</td>
                  <td className="p-3 max-w-[28rem]">
                    <span className="line-clamp-1">{q.question}</span>
                  </td>
                  <td className="p-3 whitespace-nowrap text-muted-foreground">{q.sub_category}</td>
                  <td className="p-3">
                    <DifficultyBadge difficulty={q.difficulty} />
                  </td>
                  <td className="p-3">
                    <div className="flex gap-1">
                      <Badge variant={q.show_in_daily ? 'default' : 'outline'} className="text-xs gap-0.5">
                        <BookOpen className="size-3" />
                        학습
                      </Badge>
                      <Badge variant={q.show_in_flashcard ? 'default' : 'outline'} className="text-xs gap-0.5">
            ...
            >
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                    ...
                    {q.sub_category && <span className="text-xs text-muted-foreground">· {q.sub_category}</span>}
                    <DifficultyBadge difficulty={q.difficulty} />
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  ...
                  <Button variant="ghost" size="icon" className="size-8" onClick={() => onDelete(q.id)}>
