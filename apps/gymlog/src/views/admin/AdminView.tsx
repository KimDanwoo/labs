...
export function AdminView() {
  ...
  const handleDelete = (routine: Routine) => {
    ...
    void deleteDoc(sharedRoutineDoc(routine.id)).then(() =>
      setShared((prev) => prev.filter((r) => r.id !== routine.id)),
    );
  };

  // 내 루틴을 공용 기본 루틴으로 복사(새 id, source=preset).
  const handlePromote = (routine: Routine) => {
    const promoted: Routine = { ...routine, id: crypto.randomUUID(), source: ROUTINE_SOURCE.preset };
    void setDoc(sharedRoutineDoc(promoted.id), promoted).then(() => setShared((prev) => [...prev, promoted]));
  };
      ...
      <main className="mx-auto flex w-full max-w-content flex-col gap-2xl px-lg pb-3xl pt-lg">
        <section className="flex flex-col gap-md">
          <div className="flex items-center justify-between">
            ...
            <Button variant="outline" className="h-9 px-md text-sm" onClick={handleSeedPresets}>
              PRESET 시드
            </Button>
          </div>
          {shared.length === 0 ? (
            <p className="rounded-lg border border-card-border bg-glass p-lg text-sm text-muted">
              공용 루틴이 없어요. PRESET 시드하거나 아래 내 루틴을 공용으로 올려보세요.
            </p>
          ) : (
            shared.map((routine) => (
              <div
