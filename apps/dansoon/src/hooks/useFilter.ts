...
export default function useFilter(posts: Common.Post[]) {
  ...
  const filteredPosts = (posts: Common.Post[]) => {
    const visible = posts.filter((p) => p.title !== '🧑🏻‍💻 frontend 김단우' && !p.isHidden);
  ...
  const categories = useMemo(
    () => ['ALL', ...Array.from(new Set(posts.filter((p) => p.category && !p.isHidden).map((p) => p.category)))],
    [posts],
  );
  ...
  return {
    ...
    handleClickCategory: (tag: string) => {
      if (tag) setCategory(tag);
    },
